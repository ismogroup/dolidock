...
dol_include_once('/facturx/backport/v16/core/class/html.formsetup.class.php');
...
class InterfaceFacturxTriggers extends DolibarrTriggers
{
	...
	{
		if (empty($conf->facturx) || empty($conf->facturx->enabled)) {
		...
		switch ($action) {
			...
			case 'ECMFILES_MODIFY':
				if (empty($this->getDolGlobalString('FACTURX_USE_TRIGGER',''))) {
					dol_syslog("Trigger facturx is not enabled into module setup");
					return 0;
				}
				...
				if ($object->src_object_type == 'facture' && $object->gen_or_uploaded == "generated") {
					...
					if ($res) {
						dol_syslog("Trigger facturx get details = ".json_encode($inv));
						$fx = new \custom\facturx\FacturX($this->db, $inv);
						$dir = $conf->facture->dir_output.'/'.get_exdir($inv->id, 2, 0, 0, $inv, 'invoice');
						$filename = $dir."/".$object->filename;
						dol_syslog("Trigger facturx make pdf to $filename");
						$fx->makePDF($filename);
					}
				}
				dol_syslog("Trigger facturx ".json_encode($object));
				break;
